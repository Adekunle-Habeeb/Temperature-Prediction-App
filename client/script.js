...
function onClickedEstimatePrice() {
  ...
  var url = 'https://temperature-prediction-app-23pw.onrender.com/predict_temperature_range';
  ...
  $.post(url, {
      ...
      if (parseInt(year.value) < currentYear) {
          predictionSentence.innerHTML = `The temperature for ${selectedCity} in ${monthName} ${year.value} was ${data.lower_bound.toFixed(1)}°C.`;
      } else {
          predictionSentence.innerHTML = `The temperature predicted for ${selectedCity} in ${monthName} ${year.value} is ${data.lower_bound.toFixed(1)}°C - ${data.upper_bound.toFixed(1)}°C.`;
      }
